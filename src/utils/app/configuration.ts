...
import type { KeyConfiguration } from '@/types';
...
const getKeyConfigurationFromReqHeaders = (
  req: NextApiRequest,
): KeyConfiguration => {
  ...
  const azureEmbeddingDeploymentName = req.headers[
...
const getKeyConfigurationFromEnvironment = (): KeyConfiguration => {
  ...
  const azureEmbeddingDeploymentName =
    AZURE_OPENAI_API_EMBEDDINGS_DEPLOYMENT_NAME;
...
const validateKeyConfiguration = (
  ...
  if (keyConfiguration.apiType === ModelType.AZURE_OPENAI) {
    if (
      !keyConfiguration.azureApiKey ||
      !keyConfiguration.azureInstanceName ||
      !keyConfiguration.azureApiVersion ||
